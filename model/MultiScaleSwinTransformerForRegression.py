...
class MultiScaleSwinTransformerForRegression(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 6, 2], 
        ...
        # 结合图像特征和表型特征
        self.fc1 = nn.Linear(512 + num_phenotypic_features, 512)
    ...
    def forward_features(self, x):
        ...
        pooled_features = []
        # 使用所有特征
        for feat in normalized_features:  # 修改此处
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        # print(f"PatchEmbed input shape: {x.shape}")  # 打印调试信息
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x).flatten(2).transpose(1, 2)
        # print(f"PatchEmbed after proj shape: {x.shape}")  # 打印调试信息
        if self.norm is not None:
            x = self.norm(x)
        # print(f"PatchEmbed output shape: {x.shape}")  # 打印调试信息
        return x
