...
class MultiScaleSwinTransformerForRegression(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 6, 2], 
        ...
        super(MultiScaleSwinTransformerForRegression, self).__init__()
        ...
        self.patch_embed1 = PatchEmbed(
            ...
            norm_layer=norm_layer if patch_norm else None, num_phenotypic_features=num_phenotypic_features)
        self.patch_embed2 = PatchEmbed(
            img_size=img_size, patch_size=patch_size * 2, in_chans=in_chans, embed_dim=embed_dim * 2,
            norm_layer=norm_layer if patch_norm else None, num_phenotypic_features=num_phenotypic_features)
        self.patch_embed3 = PatchEmbed(
            img_size=img_size, patch_size=patch_size * 4, in_chans=in_chans, embed_dim=embed_dim * 4,
            norm_layer=norm_layer if patch_norm else None, num_phenotypic_features=num_phenotypic_features)
    ...
    def forward_features(self, x, phenotypes):

        # 多尺度特征提取，包含表型特征
        features = [
            self.patch_embed1(x, phenotypes),
            self.patch_embed2(x, phenotypes),
            self.patch_embed3(x, phenotypes)
        ]
        ...
        for feat in normalized_features:
            pooled = self.avgpool(feat.transpose(1, 2)).flatten(1)
    ...
    def forward(self, x, phenotypes):
        combined_features = self.forward_features(x, phenotypes)
    ...
    def flops(self):
        ...
        # 表型特征嵌入的 FLOPs
        flops += self.phenotype_embed.in_features * self.phenotype_embed.out_features
        return flops

class PatchEmbed(nn.Module):
    """Image to Patch Embedding with Phenotype Feature Integration
    ...
    Args:
        ...
        num_phenotypic_features (int): Number of phenotypic features. Default: 8.
    """
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, num_phenotypic_features=8):
        super().__init__()
    ...
    def forward(self, x, phenotypes):
        ...
        # 嵌入表型特征
        phenotype_embedded = self.phenotype_embed(phenotypes).unsqueeze(1)  # B, 1, C
        
        # 将表型特征广播并加到所有patch上
        x = x + phenotype_embedded
    ...
    def flops(self):
        ...
        if self.norm is not None:
            flops += (Ho * Wo + 1) * self.embed_dim  # +1 for phenotype feature
        flops += self.phenotype_embed.in_features * self.phenotype_embed.out_features
        return flops
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        # print("BasicLayer input shape:", x.shape)
        ...
        if self.downsample is not None:
            x = self.downsample(x)
        # print("BasicLayer output shape:", x.shape)
        return x
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        H, W = self.input_resolution
        B, L, C = x.shape
        # print(f"SwinTransformerBlock input shape: {x.shape}, Expected shape: {H * W}")
    ...
    def flops(self):
        flops = 0
        H, W = self.input_resolution
        flops += H * W * self.dim * 2  # norm1
        nW = (H // self.window_size) * (W // self.window_size)
        flops += nW * self.window_size * self.window_size * self.dim * self.num_heads  # qkv
        flops += nW * self.window_size * self.window_size * self.dim * self.num_heads  # attn
        flops += nW * self.window_size * self.window_size * self.dim * self.dim  # proj
        flops += H * W * self.dim * self.dim * 4  # mlp hidden
        flops += H * W * self.dim * self.dim  # mlp output
        return flops
...
class WindowAttention(nn.Module):
    ...
    Args:
        ...
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
        qk_scale (float | None, optional): Override default qk scale of head_dim ** -0.5 if set.
        attn_drop (float, optional): Dropout rate of attention. Default: 0.0
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B // nW, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0).to(attn.device)
    ...
    def flops(self):
        ...
        flops += self.num_heads * N * (N + 1) * self.dim // self.num_heads  # attention
...
class Mlp(nn.Module):
    ...
    def flops(self, num_tokens):
        flops = 0
        flops += num_tokens * self.fc1.in_features * self.fc1.out_features
        flops += num_tokens * self.fc2.in_features * self.fc2.out_features
        return flops
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        B, L, C = x.shape
        assert L == H * W, f"Input feature has wrong size: {L} != {H} * {W}"
        assert H % 2 == 0 and W % 2 == 0, f"Input resolution must be even, but got ({H}, {W})"
